          ...
          <div class="table-responsive">
            <table class="table info_table table-hover" id="accounts_table">
              ...
              <?php foreach($customers_list as $customer) :
						...
						if($customer->customer_data_status==1){$label='label-success';$status='Activer';}
							if($customer->customer_data_status==0){$label='label-danger';$status='Désactiver';}
               ...
               <tr class="supp-<?php  echo $customer->customer_id;?>"> 
						...
						<td class="sub_col" > <span style="position: relative;top:5px;text-align: left;">   <?php echo $customer->customer_phone;?> </span> </td>
                       <td class="sub_col" > <span style="position: relative;top:5px;"> <?php echo $customer->customer_adress.' '. $customer->customer_zip.' '. $customer->customer_city;?> </span> </td>
					<td class="sub_col" ><span >
						
						<a href="<?php echo base_url().'orders/customers/'.$customer->customer_id;?>"><button type="button" class="btn btn-outline btn-primary"><i class="glyphicon glyphicon-eye-open"></i></button></a>
				...
				<div role="group" aria-label="Basic example" class="btn-group btn-group-sm">
									<select  id="ddlCreditCardTypeTab" data-id="<?php echo $customer->customer_id;?>" name="ddlCreditCardTypeTab" class="form-control changesatus btn-xs">
										<option <?php  if($customer->customer_data_status==1){ echo"selected";} ?> value="1">Activer</option>
										<option <?php  if($customer->customer_data_status==0){ echo"selected";} ?> value="0">Désactiver</option>
									</select>
									</div>
