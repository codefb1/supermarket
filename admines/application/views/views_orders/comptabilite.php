<div class="col-sm-12">
  ...
  <h2 class="page_title" style="text-align: center;">Comptabilité </h2>
 
</div>
<div class="col-sm-12 text-center">
    <button class="btn btn-shadow btn-success" type="button" onClick="location.href='#'"> Factures de vente </button>
 <button class="btn btn-shadow btn-primary" type="button" onClick="location.href='<?php echo base_url().'comptabilite/partners';?>'"><i class="icon-plus"></i> Factures achat  fournisseur </button>
 <button class="btn btn-shadow btn-primary" type="button" onClick="location.href='<?php echo base_url().'comptabilite/transporters';?>'"><i class="icon-plus"></i> Factures achat livreurs</button>
  ...
  <div class="container-fluid">
			  ...
			  $total = $orders->order_amount+$orders->order_shipping_rate;
			   ...
			   Facture  <?php  echo $orders->order_id;?>
			   		...
			   		<td class="sub_col" >   <span class="position_text"> <?php echo $orders->order_reference;?> </span> </td>
							...
							<td class="sub_col" > <span class="position_text"> <?php echo (number_format($orders->order_total_marge, 2, ',', ''))."  euro" ?></span> </td>
	...
	<td class="sub_col" > <span class="position_text">  <div> <?php echo date_fr($orders->order_data_created);?> </div><div> <?php echo date_fr_heur($orders->order_data_created);?></div> </span> </td>
				...
				<td class="sub_col">
				        	...
				        	<a href="<?php echo base_url().'comptabilite/orderPdf/'.$orders->order_id;?>" class="position_text"><button type="button" class="btn btn-outline btn-primary"><i class=" glyphicon glyphicon-eye-open"></i></button></a>
